...
BACK_RE  = re.compile(r"^(回来|回|back)$", re.IGNORECASE)
...
async def begin(update: Update, ctx: ContextTypes.DEFAULT_TYPE, kind: str):
    ...
    ud = ctx.user_data
    if ud.get("active"):
        await msg.reply_html(f"{mention_user_html(user)} 已有进行中的打卡，请先发送“回来/回/back”或 /back 结束。")
        return
    ...
    if limit_count and today_count >= limit_count:
        await msg.reply_html(f"{mention_user_html(user)} 本{current_shift_label()}次数已达上限 <b>{limit_count}</b> 次。")
        return
    ...
    if last_end_ts:
        ...
        if delta_min < COOLDOWN_MIN.get(kind, 0):
            ...
            await msg.reply_html(f"{mention_user_html(user)} 刚结束不久，{TITLES[kind]} 冷却 <b>{need}</b> 分钟内请勿重复开始。")
            return

    limit = LIMITS[kind]
    ud["active"] = {"type": kind, "title": TITLES[kind], "start": datetime.now(timezone.utc), "limit": limit}
    ...
    ud["reminder_job"] = ctx.job_queue.run_once(
        remind_timeout, when=run_at, data={"uid": user.id, "chat_id": chat.id}, name=f"remind-{user.id}"
    )
    # 宽限后提醒管理员（@）
    ud["grace_job"] = ctx.job_queue.run_once(
        remind_grace, when=run_at + timedelta(minutes=GRACE_MINUTES),
        data={"uid": user.id, "chat_id": chat.id}, name=f"grace-{user.id}"
    )
...
async def end_session(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not active:
        await msg.reply_html(f"{mention_user_html(user)} 当前没有进行中的打卡。")
        return
...
async def remind_timeout(ctx: ContextTypes.DEFAULT_TYPE):
    data = ctx.job.data or {}
    uid = data.get("uid"); chat_id = data.get("chat_id")
    if uid is None or chat_id is None:
        return
    active = ctx.user_data.get("active")
    if not active:
        return
    limit_min = int(active.get("limit", 0))
    await ctx.bot.send_message(
        chat_id=chat_id,
        text=(f"⏰ <a href=\"tg://user?id={uid}\">你</a> 的已到上限 {limit_min} 分，"
              f"若还未结束请尽快发送“回来/回/back”或 /back。"),
        parse_mode=constants.ParseMode.HTML
    )

async def remind_grace(ctx: ContextTypes.DEFAULT_TYPE):
    data = ctx.job.data or {}
    uid = data.get("uid"); chat_id = data.get("chat_id")
    if uid is None or chat_id is None:
        return
    ...
    manager_mention = mention_id_html(MANAGER_ID, f"@{MANAGER_NAME}")
    await ctx.bot.send_message(
        chat_id=chat_id,
        text=(f"⚠️ {manager_mention} 提醒：<a href=\"tg://user?id={uid}\">这位同事</a> 的 {title} "
              f"已超过上限并宽限 <b>{GRACE_MINUTES}</b> 分钟仍未结束，当前已用时 <b>{used}</b>。"),
        parse_mode=constants.ParseMode.HTML,
        disable_web_page_preview=True
    )
...
async def cmd_who(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    chat = update.effective_chat; app = ctx.application; now_utc = datetime.now(timezone.utc)
    lines = []
    for uid, ud in list(app.user_data.items()):
        active = ud.get("active")
...
async def cmd_summary(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update):
        return await update.effective_message.reply_html("❌ 仅管理员可用。")
    app = ctx.application; lines = [f"📊 本{current_shift_label()}汇总（按用户）："]
    for uid, ud in list(app.user_data.items()):
        stats = ud.get("stats") or {}
        per = []
        for k in ("smoke", "toilet", "meal"):
            c = stats.get(k, {}).get("count", 0); d = stats.get(k, {}).get("dur", 0)
            if c or d:
...
async def cmd_setlimit(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update):
        return await update.effective_message.reply_html("❌ 仅管理员可用。")
    try: name, minutes = ctx.args[0], int(ctx.args[1])
    except Exception: return await update.effective_message.reply_html("用法：/setlimit 抽烟 12")
    key = next((k for k, v in TITLES.items() if v == name), None)
    if not key: return await update.effective_message.reply_html("类型不对：厕所/抽烟/吃饭")
    LIMITS[key] = minutes
...
async def cmd_setcount(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update):
        return await update.effective_message.reply_html("❌ 仅管理员可用。")
    try: name, cnt = ctx.args[0], int(ctx.args[1])
    except Exception: return await update.effective_message.reply_html("用法：/setcount 抽烟 2")
    key = next((k for k, v in TITLES.items() if v == name), None)
    if not key: return await update.effective_message.reply_html("类型不对：厕所/抽烟/吃饭")
...
async def cmd_unmute(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_html("🔔 已取消静音。")
...
def main():
    if not BOT_TOKEN or BOT_TOKEN == "PUT_YOUR_TELEGRAM_BOT_TOKEN_HERE":
        raise RuntimeError("缺少 BOT_TOKEN：请设置环境变量 BOT_TOKEN。")
    ...
    app: Application = (
        ...
        .post_init(setup_bot_commands)  # 启动时设置 / 菜单
        .build()
    )
